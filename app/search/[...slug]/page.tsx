...
import { CharacterInfo } from '@/app/api/getCharacters/route';
...
function SearchPage({ slug }: { slug: string }) {
  ...
  const { isPending, error, data } = useQuery<{ data: CharacterInfo }>({
    queryKey: ['repoData'],
    queryFn: () =>
      fetch('/api/getCharacters?query=' + search).then((res) =>
        res.json(),
      ),
  });
